...
const Banner = ({ data }) => {
  return (
    <div className='min-h-100 flex justify-center items-center py-12 bg-gray-900'>
      <div 
        className='container'>
        <div style={{backgroundColor: data.bgColor}}
         className='grid grid-cols-1 md:grid-cols-3 gap-6 items-center text-white rounded-3xl'>
            ...
            <div data-aos='zoom-in' className='h-full flex items-center '>
                <img src={data.image} alt="" className='scale-125 w-62.5 md:w-85 mx-auto drop-shadow-2xl object-cover ' />
            </div>
            {/* third col */}
            <div className='flex flex-col justify-center gap-4 p-6 sm:p-8'>
                <p data-aos='zoom-out' className='font-bold text-xl text-w'>{data.title2}</p>
                ...
                <div data-aos='fade-up' data-aos-offset='0' >
                    <button style={{color: data.bgColor}}
                     className='bg-white text-black py-2 px-4 rounded-full '>Shop Now</button>
                </div>
            </div>
        </div>
      </div>
